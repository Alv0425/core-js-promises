...
function getPromiseResult(source) {
  return Promise.resolve(source.then(() => 'success').catch(() => 'fail'));
}
...
function getAllResult(promises) {
  return Promise.all(
    promises.map((promise) =>
      promise.then((result) => result).catch(() => Promise.resolve(null))
    )
  );
}
...
async function queuPromises(promises) {
  const result = await promises.reduce(async (accum, promise) => {
    const sum = (await accum) + (await promise);
    return sum;
  }, '');
  return result;
}
